...
public class Organism extends Cell {
	...
	public int choose() {
		...
		int maternalInc;
		int act;
		if (this.energy < preservation[0]*10) {
		...
		do {
			...
			temp = (int) (growthLim*2 - this.diam)/5 + (this.energy/1000) * multiplier;
			if(temp < 0) {
				temp = 0;
			}
			total += temp;
			...
			temp = (this.energy - maternalM*10) * multiplier;
			if(temp < 0) {
				temp = 0;
			}
			total += temp;
	...
	public Organism split(){
		...
		Color newColor = new Color(Math.abs(((int)(newTendencies[2]*5))%255), Math.abs(((int)(newTendencies[5]*5))%255), Math.abs(((int)(newTendencies[6]*5))%255));
